# isma_talent_analyzer.py  (fixed: no reorderable kwarg)
...
# ───────── STEP 4 : Craft Unique Social Contribution ────────────────────────
if step == 4:
    st.subheader("4️⃣ Craft Unique Social Contribution (Over-Capacity)")

    df_tal = pd.DataFrame({"Talent": st.session_state["uoms"]})
    df_ver = st.session_state.get(
        "verbs_df", pd.DataFrame(columns=["Use", "Talent", "Verb"])
    )

    st.markdown("**Reorder super-talents (top → highest priority)**")
    df_tal = st.data_editor(df_tal, num_rows="dynamic", use_container_width=True)
